...
/**
 * @brief Function to retrieve the 4 first bytes from ESP device.
 * 
 */
void get_dw1000_id ( uint8_t * id ) {
    char uidstr[12];

    uint32_t id32 = HAL_GetUIDw0();
    memcpy( &uidstr[8], &id32, 4 );
    id32 = HAL_GetUIDw1();
    memcpy( &uidstr[4], &id32, 4 );
    id32 = HAL_GetUIDw2();
    memcpy( &uidstr[0], &id32, 4 );

    uint32_t id_hash = Hash32Len5to12( (const char *)uidstr, 12 );
    uint8_t id8[2];

    for ( int i = 0; i < 4; i++ ) {
        id8[i] = (id_hash >> (8 * (3 - i)) ) & 0xFF;
    }

    memcpy( id, id8, 2 );
}

uint8_t own_id[2] = { 0, 0 };

/**
 * @brief DW1000 two-way ranging Setupt function
 * 
 */
 void uwb_setup(){
    ...
    /* Get own 2 bytes ID */
    get_dw1000_id ( own_id );

    /* Sets the tag own ID for transmitted and received data*/
    // First, the transmitter
    rx_poll_msg[ RX_OWN_ADD_IDX_1 ] = own_id[0];
    rx_poll_msg[ RX_OWN_ADD_IDX_2 ] = own_id[1];
    // --
    tx_resp_msg[ TX_OWN_ADD_IDX_1 ] = own_id[0];
    tx_resp_msg[ TX_OWN_ADD_IDX_2 ] = own_id[1];

    // Then the received
    rx_final_msg[ RX_OWN_ADD_IDX_1 ] = own_id[0];
    rx_final_msg[ RX_OWN_ADD_IDX_2 ] = own_id[1];

    /* Also, sets its own ID for the ping and distance fdbk message */
    tx_ping[ 1 ] = own_id[0];
    tx_ping[ 2 ] = own_id[1];

    tx_distance_fdbk[ 3 ] = own_id[0];
    tx_distance_fdbk[ 4 ] = own_id[1];
 }


static int32_t time_to_ping = 0;
 /**
  * @brief DW1000 loop function
  * 
  */
 void uwb_loop(){
